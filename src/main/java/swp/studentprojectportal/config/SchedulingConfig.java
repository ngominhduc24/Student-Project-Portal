...
public class SchedulingConfig {
    ...
    public void syncGitlabConfig() {
        System.out.println("Sync gitlab config");
        List<Class> classList = classService.findAll();
        System.out.println(classList.size());
        for (Class aClass : classList) {
            if (aClass.getGitlabGroupId() == null) {
                continue;
            }
            try {
                synchronizeGitlabMilestoneClass(
                        aClass.getId(),
                        aClass.getGitlabGroupId(),
                        aClass.getUser().getPersonalTokenGitlab()
                );
                System.out.println("Sync milestone class " + aClass.getId() + " success");
